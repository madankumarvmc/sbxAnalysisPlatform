#!/usr/bin/env python3
"""
Warehouse Analysis Tool V2 - Streamlit Application (Simplified)

A streamlined tool for warehouse data analysis with Excel-first output approach.
Upload your Excel file, configure variables, run analysis, and download comprehensive reports.
"""

import streamlit as st
import pandas as pd
import io
import os
from datetime import datetime
import traceback

# Import configuration and analysis modules
import config
from scripts import (
    DataLoader,
    OrderAnalyzer,
    SKUAnalyzer,
    ABCFMSAnalyzer,
    ReceiptAnalyzer,
    InventoryAnalyzer,
    ManpowerAnalyzer,
    ExcelGenerator
)

# Configure page
st.set_page_config(
    page_title="Warehouse Analysis Tool V2",
    page_icon="🏢",
    layout="wide",
    initial_sidebar_state="expanded"
)

# Custom CSS for professional styling
st.markdown("""
<style>
    .main-header {
        font-size: 2.5rem;
        font-weight: 700;
        color: #1f77b4;
        text-align: center;
        margin-bottom: 1rem;
    }
    .section-header {
        font-size: 1.3rem;
        font-weight: 600;
        color: #2e86ab;
        margin-top: 1.5rem;
        margin-bottom: 0.8rem;
    }
    .info-box {
        background-color: #e8f4fd;
        padding: 0.8rem;
        border-radius: 0.3rem;
        border-left: 4px solid #1f77b4;
        margin: 0.8rem 0;
    }
    .success-box {
        background-color: #d4edda;
        padding: 0.8rem;
        border-radius: 0.3rem;
        border-left: 4px solid #28a745;
        margin: 0.8rem 0;
    }
    .error-box {
        background-color: #f8d7da;
        padding: 0.8rem;
        border-radius: 0.3rem;
        border-left: 4px solid #dc3545;
        margin: 0.8rem 0;
    }
    .metric-container {
        background-color: #f8f9fa;
        padding: 1rem;
        border-radius: 0.3rem;
        border: 1px solid #dee2e6;
        margin: 0.5rem 0;
    }
</style>
""", unsafe_allow_html=True)

# Step labels — single source of truth
STEPS = [
    "📁 Upload Excel File",
    "⚙️ Configure Variables",
    "🚀 Run Analysis",
    "📥 Download Results"
]


def main():
    """Main Streamlit application"""

    # Header
    st.markdown('<h1 class="main-header">🏢 Warehouse Analysis Tool V2</h1>', unsafe_allow_html=True)
    st.markdown("""
    <div class="info-box">
        <strong>Excel-First Analysis Tool</strong><br>
        Upload your warehouse data Excel file, configure analysis parameters, and generate comprehensive Excel reports.
    </div>
    """, unsafe_allow_html=True)

    # Initialize session state
    initialize_session_state()

    # Sidebar navigation — index seeded from requested_step or inferred from state
    with st.sidebar:
        st.markdown("## 🏢 Warehouse Analysis")

        if st.session_state.requested_step is not None:
            default_index = st.session_state.requested_step
            st.session_state.requested_step = None   # consume
        else:
            if not st.session_state.file_validated:
                default_index = 0
            elif not st.session_state.variables_configured:
                default_index = 1
            elif not st.session_state.analysis_complete:
                default_index = 2
            else:
                default_index = 3

        selected_step = st.radio(
            "Analysis Steps:",
            STEPS,
            index=default_index,
            key="navigation_step"
        )

        # Show progress indicator
        show_progress_indicator()

    # Main content based on selected step
    if selected_step == STEPS[0]:
        show_upload_section()
    elif selected_step == STEPS[1]:
        show_variables_section()
    elif selected_step == STEPS[2]:
        show_analysis_section()
    elif selected_step == STEPS[3]:
        show_download_section()


def initialize_session_state():
    """Initialize all session state variables"""

    # File upload state
    if 'uploaded_file' not in st.session_state:
        st.session_state.uploaded_file = None
    if 'file_sheets' not in st.session_state:
        st.session_state.file_sheets = {}
    if 'file_validated' not in st.session_state:
        st.session_state.file_validated = False

    # Variable configuration state
    if 'variables_configured' not in st.session_state:
        st.session_state.variables_configured = False
    if 'analysis_variables' not in st.session_state:
        st.session_state.analysis_variables = config.get_variable_defaults()

    # Analysis state
    if 'analysis_complete' not in st.session_state:
        st.session_state.analysis_complete = False
    if 'analysis_results' not in st.session_state:
        st.session_state.analysis_results = None
    if 'excel_report_ready' not in st.session_state:
        st.session_state.excel_report_ready = False

    # New keys (Step 16)
    if 'loaded_data' not in st.session_state:
        st.session_state.loaded_data = None          # cached DataFrames — avoids double-load
    if 'analysis_status' not in st.session_state:
        st.session_state.analysis_status = {}        # {name: True/False}
    if 'analysis_errors' not in st.session_state:
        st.session_state.analysis_errors = {}        # {name: error_string}
    if 'word_report_buffer' not in st.session_state:
        st.session_state.word_report_buffer = None
    if 'word_report_filename' not in st.session_state:
        st.session_state.word_report_filename = None
    if 'requested_step' not in st.session_state:
        st.session_state.requested_step = None       # int 0-3; consumed once by main()


def reset_analysis(full_reset=False):
    """Clear analysis state. Pass full_reset=True to also clear file and variable state."""
    st.session_state.analysis_complete    = False
    st.session_state.analysis_results     = None
    st.session_state.excel_report_ready   = False
    st.session_state.analysis_status      = {}
    st.session_state.analysis_errors      = {}
    st.session_state.word_report_buffer   = None
    st.session_state.word_report_filename = None
    if full_reset:
        st.session_state.uploaded_file        = None
        st.session_state.file_sheets          = {}
        st.session_state.file_validated       = False
        st.session_state.loaded_data          = None
        st.session_state.variables_configured = False
        st.session_state.analysis_variables   = config.get_variable_defaults()


def show_progress_indicator():
    """Show compact progress indicator in sidebar"""
    st.markdown("### Progress")
    steps_state = [
        ("File Upload",  st.session_state.file_validated),
        ("Variables",    st.session_state.variables_configured),
        ("Analysis",     st.session_state.analysis_complete),
        ("Download",     st.session_state.excel_report_ready),
    ]
    for label, done in steps_state:
        icon = "✅" if done else "○"
        st.markdown(f"{icon} {label}")

    # Failure alert — visible without leaving sidebar
    failed = [k for k, v in st.session_state.get('analysis_status', {}).items() if not v]
    if failed:
        st.warning(f"{len(failed)} module(s) had errors — see Run Analysis")


def show_step_navigation_footer(step_index: int, next_label: str = None, next_disabled: bool = False):
    """
    Render navigation footer at the bottom of each section.
    Left: Reset / Start Over (when step_index > 0).
    Right: Next: {next_label} → (when next_label is provided).
    """
    st.markdown("---")
    col_left, col_right = st.columns([1, 2])

    with col_left:
        if step_index > 0:
            if st.button("Reset / Start Over", type="secondary", use_container_width=True):
                reset_analysis(full_reset=True)
                st.session_state.requested_step = 0
                st.rerun()

    with col_right:
        if next_label is not None:
            if st.button(
                f"Next: {next_label} →",
                type="primary",
                disabled=next_disabled,
                use_container_width=True
            ):
                st.session_state.requested_step = step_index + 1
                st.rerun()


# ──────────────────────────────────────────────────────────────────────────────
# SECTION 1 — Upload
# ──────────────────────────────────────────────────────────────────────────────

def show_upload_section():
    """Display the Excel file upload section"""
    st.markdown('<h2 class="section-header">📁 Upload Excel File</h2>', unsafe_allow_html=True)

    st.markdown("""
    Upload your warehouse data Excel file containing the required sheets:
    - **OrderData**: Order transactions and shipment details
    - **SkuMaster**: Product information and categories
    - **InventoryData**: Stock levels and inventory details
    - **ReceiptData**: Receipt and inbound data
    """)

    # Template download and file upload in columns
    col1, col2 = st.columns([1, 2])

    with col1:
        st.markdown("**📄 Need sample data?**")
        if st.button("📥 Download Sample Data", type="secondary", use_container_width=True):
            template_buffer = generate_template_excel()
            st.download_button(
                label="📄 Download Sample Excel Data",
                data=template_buffer,
                file_name="Sample_Warehouse_Data.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                use_container_width=True
            )

        st.markdown("""
        <div style="font-size: 0.8rem; color: #666; margin-top: 0.5rem;">
        Template includes:<br>
        • OrderData (100 sample orders)<br>
        • SkuMaster (50 SKUs)<br>
        • ReceiptData (75 receipts)<br>
        • InventoryData (80 records)<br>
        With realistic sample data matching your format.
        </div>
        """, unsafe_allow_html=True)

    with col2:
        uploaded_file = st.file_uploader(
            "Choose your warehouse Excel file",
            type=['xlsx', 'xls'],
            help="Upload an Excel file containing your warehouse data sheets",
            key="main_file_uploader"
        )

    if uploaded_file is not None:
        st.session_state.uploaded_file = uploaded_file
        process_uploaded_file(uploaded_file)

    # Persistent confirmation — shown on every render when file is validated
    if st.session_state.file_validated:
        st.success("File loaded and validated. Ready to configure and analyse.")
        show_file_summary()
        show_data_availability_summary()

    show_step_navigation_footer(
        0, "Configure Variables",
        next_disabled=not st.session_state.file_validated
    )


def process_uploaded_file(uploaded_file):
    """Process and validate the uploaded Excel file"""
    try:
        # Clear any stale analysis results when a new file is uploaded
        reset_analysis(full_reset=False)

        with st.spinner("Reading and validating your Excel file..."):
            data_loader = DataLoader(uploaded_file, verbose=False)
            load_results = data_loader.load_all_data()

        if load_results['success']:
            # Cache the loaded DataFrames — avoids re-loading on analysis run
            st.session_state.loaded_data = load_results['data']

            st.session_state.file_sheets = {}
            for data_type, data_df in load_results['data'].items():
                st.session_state.file_sheets[data_type] = {
                    'rows': len(data_df),
                    'columns': list(data_df.columns)
                }

            st.session_state.file_validated = True
        else:
            st.error("File processing failed:")
            for error in load_results['errors']:
                st.error(f"• {error}")
            st.session_state.file_validated = False

    except Exception as e:
        st.error(f"Error processing file: {str(e)}")
        st.session_state.file_validated = False


def show_file_summary():
    """Display summary of uploaded file"""
    st.markdown("### 📋 File Summary")

    if not st.session_state.file_sheets:
        return

    col1, col2 = st.columns(2)

    with col1:
        st.markdown("**Detected Sheets:**")
        for sheet_name, sheet_info in st.session_state.file_sheets.items():
            st.write(f"✅ {sheet_name.replace('_', ' ').title()}: {sheet_info['rows']} rows")

    with col2:
        sheet_to_preview = st.selectbox(
            "Preview sheet columns:",
            list(st.session_state.file_sheets.keys()),
            key="sheet_preview_selector",
            format_func=lambda x: x.replace('_', ' ').title()
        )

        if sheet_to_preview:
            st.write(f"**Columns in '{sheet_to_preview.replace('_', ' ').title()}':**")
            columns = st.session_state.file_sheets[sheet_to_preview]['columns']
            for i, col in enumerate(columns[:10], 1):
                st.write(f"{i}. {col}")
            if len(columns) > 10:
                st.write(f"... and {len(columns) - 10} more columns")


def show_data_availability_summary():
    """Show which analysis modules will run based on detected sheets."""
    availability = config.validate_sheet_availability(st.session_state.file_sheets)

    has_manpower = (
        'order_data' in st.session_state.file_sheets or
        'receipt_data' in st.session_state.file_sheets
    )

    module_info = [
        ('order_analysis',    'Order Pattern Analysis',    'OrderData'),
        ('sku_analysis',      'SKU Performance Analysis',  'OrderData + SkuMaster'),
        ('abc_fms_analysis',  'ABC-FMS Classification',    'OrderData + SkuMaster'),
        ('inventory_analysis','Inventory Analysis',        'InventoryData'),
        ('receipt_analysis',  'Receipt Analysis',          'ReceiptData'),
    ]

    st.markdown("#### What will be analysed")
    cols = st.columns(3)
    for i, (key, label, required) in enumerate(module_info):
        with cols[i % 3]:
            if availability.get(key, False):
                st.success(f"**{label}**\n\n_{required}_")
            else:
                st.warning(f"**{label}** — skipped\n\n_Needs: {required}_")

    # Manpower (6th tile)
    with cols[len(module_info) % 3]:
        if has_manpower:
            st.success("**Manpower Analysis**\n\n_OrderData or ReceiptData_")
        else:
            st.warning("**Manpower Analysis** — skipped\n\n_Needs: OrderData or ReceiptData_")


# ──────────────────────────────────────────────────────────────────────────────
# SECTION 2 — Configure Variables
# ──────────────────────────────────────────────────────────────────────────────

def show_variables_section():
    """Display the variable configuration section"""
    st.markdown('<h2 class="section-header">⚙️ Configure Variables</h2>', unsafe_allow_html=True)

    if not st.session_state.file_validated:
        st.warning("Please upload and validate an Excel file first.")
        show_step_navigation_footer(1, "Run Analysis", next_disabled=True)
        return

    # Quick-start path — skip form entirely
    if not st.session_state.variables_configured:
        st.info(
            "**Quick start:** The default settings work well for most warehouses. "
            "You can always re-run with custom settings later."
        )
        if st.button("Use Default Settings →", type="primary"):
            st.session_state.variables_configured = True
            st.session_state.requested_step = 2
            st.rerun()
        st.markdown("Or scroll down to customise the parameters.")
        st.markdown("---")

    # Configuration form
    with st.form("variables_form"):
        create_variables_form()

        submitted = st.form_submit_button("💾 Save Configuration", type="primary")

        if submitted:
            save_variables_configuration()

    show_step_navigation_footer(
        1, "Run Analysis",
        next_disabled=not st.session_state.file_validated
    )


def create_variables_form():
    """Create the unified variables configuration form"""

    # Ensure sub-dicts exist
    if 'receipt_analysis' not in st.session_state.analysis_variables:
        st.session_state.analysis_variables['receipt_analysis'] = {
            'percentile_levels': [95, 90, 85, 80, 75]
        }
    if 'manpower_analysis' not in st.session_state.analysis_variables:
        st.session_state.analysis_variables['manpower_analysis'] = config.DEFAULT_MANPOWER_ANALYSIS_PARAMS.copy()
        st.session_state.analysis_variables['manpower_analysis'].update({
            'target_efficiency': 85,
            'standard_pick_rate': 60,
            'shifts_per_day': 1,
            'break_time_minutes': 30
        })
    for section in ['picking_simplified', 'receiving_simplified', 'picking', 'receiving_putaway', 'loading']:
        if section not in st.session_state.analysis_variables['manpower_analysis']:
            st.session_state.analysis_variables['manpower_analysis'][section] = config.DEFAULT_MANPOWER_ANALYSIS_PARAMS[section].copy()

    # ── 1. Classification Thresholds ────────────────────────────────────────
    with st.expander("📊 Classification Thresholds (ABC / FMS)", expanded=True):
        col1, col2 = st.columns(2)

        with col1:
            st.markdown("**ABC Classification**")
            abc_a_threshold = st.slider(
                "A Items Threshold (%)",
                min_value=50, max_value=90,
                value=st.session_state.analysis_variables['global']['abc_a_threshold'],
                step=5,
                key="abc_a_threshold"
            )
            abc_b_threshold = st.slider(
                "B Items Threshold (%)",
                min_value=abc_a_threshold, max_value=95,
                value=st.session_state.analysis_variables['global']['abc_b_threshold'],
                step=5,
                key="abc_b_threshold"
            )

        with col2:
            st.markdown("**FMS Classification**")
            fms_fast_threshold = st.slider(
                "Fast Items Threshold (%)",
                min_value=50, max_value=90,
                value=st.session_state.analysis_variables['order_analysis']['fms_fast_threshold'],
                step=5,
                key="fms_fast_threshold"
            )
            fms_medium_threshold = st.slider(
                "Medium Items Threshold (%)",
                min_value=fms_fast_threshold, max_value=95,
                value=st.session_state.analysis_variables['order_analysis']['fms_medium_threshold'],
                step=5,
                key="fms_medium_threshold"
            )

    # ── 2. Manpower Planning ─────────────────────────────────────────────────
    with st.expander("⚡ Manpower Planning", expanded=True):

        st.markdown("### ⚙️ Calculation Complexity")
        complexity_mode = st.selectbox(
            "Manpower Calculation Complexity",
            options=['Simple', 'Detailed'],
            index=0 if st.session_state.analysis_variables['manpower_analysis'].get('complexity_mode', 'Simple') == 'Simple' else 1,
            help="Simple: use average time per pallet. Detailed: use granular time-study inputs.",
            key="complexity_mode"
        )

        if complexity_mode == 'Simple':
            st.markdown("### 🔄 Picking")
            col1, col2 = st.columns(2)
            with col1:
                percentile_planning = st.slider(
                    "Percentile for Planning (%)",
                    min_value=70, max_value=99,
                    value=st.session_state.analysis_variables['manpower_analysis'].get('picking_simplified', {}).get('percentile_for_planning', 95),
                    help="Use this percentile of daily volume for staffing calculations",
                    key="percentile_planning"
                )
                cases_per_pallet = st.number_input(
                    "Average Cases per Pallet",
                    min_value=10, max_value=500,
                    value=st.session_state.analysis_variables['manpower_analysis'].get('picking_simplified', {}).get('average_cases_per_pallet', 100),
                    step=5,
                    key="cases_per_pallet"
                )
                time_per_pallet = st.number_input(
                    "Average Time per Pallet (minutes)",
                    min_value=5.0, max_value=120.0,
                    value=st.session_state.analysis_variables['manpower_analysis'].get('picking_simplified', {}).get('average_time_per_pallet', 30.0),
                    step=1.0,
                    key="time_per_pallet"
                )
            with col2:
                work_efficiency = st.slider(
                    "Work Efficiency (%)",
                    min_value=50, max_value=100,
                    value=int(st.session_state.analysis_variables['manpower_analysis'].get('picking_simplified', {}).get('work_efficiency', 85)),
                    help="Actual productivity as percentage of theoretical time",
                    key="work_efficiency"
                )
                shift_hours = st.number_input(
                    "Shift Hours",
                    min_value=4.0, max_value=12.0,
                    value=st.session_state.analysis_variables['manpower_analysis'].get('picking_simplified', {}).get('shift_hours', 8.0),
                    step=0.5,
                    key="shift_hours"
                )
                break_time = st.number_input(
                    "Break Time (minutes)",
                    min_value=0, max_value=120,
                    value=st.session_state.analysis_variables['manpower_analysis'].get('picking_simplified', {}).get('break_time_minutes', 30),
                    step=5,
                    key="break_time"
                )

            st.markdown("### 📦 Receiving")
            col1, col2 = st.columns(2)
            with col1:
                receiving_percentile = st.slider(
                    "Percentile for Receiving Planning (%)",
                    min_value=70, max_value=99,
                    value=st.session_state.analysis_variables['manpower_analysis'].get('receiving_simplified', {}).get('percentile_for_planning', 95),
                    help="Use this percentile of daily receipt volume for staffing calculations",
                    key="receiving_percentile_planning"
                )
                receiving_cases_per_pallet = st.number_input(
                    "Average Cases per Pallet (Receiving)",
                    min_value=10, max_value=500,
                    value=st.session_state.analysis_variables['manpower_analysis'].get('receiving_simplified', {}).get('average_cases_per_pallet', 100),
                    step=5,
                    key="receiving_cases_per_pallet"
                )
                unloading_putaway_time = st.number_input(
                    "Unloading + Putaway Time per Pallet (minutes)",
                    min_value=10.0, max_value=180.0,
                    value=st.session_state.analysis_variables['manpower_analysis'].get('receiving_simplified', {}).get('unloading_putaway_time_per_pallet', 45.0),
                    step=1.0,
                    help="Includes unloading from truck + scanning + putaway to location",
                    key="unloading_putaway_time"
                )
            with col2:
                receiving_efficiency = st.slider(
                    "Receiving Work Efficiency (%)",
                    min_value=50, max_value=100,
                    value=int(st.session_state.analysis_variables['manpower_analysis'].get('receiving_simplified', {}).get('work_efficiency', 85)),
                    key="receiving_efficiency"
                )
                receiving_shift_hours = st.number_input(
                    "Receiving Shift Hours",
                    min_value=4.0, max_value=12.0,
                    value=st.session_state.analysis_variables['manpower_analysis'].get('receiving_simplified', {}).get('shift_hours', 8.0),
                    step=0.5,
                    key="receiving_shift_hours"
                )
                receiving_break_time = st.number_input(
                    "Receiving Break Time (minutes)",
                    min_value=0, max_value=120,
                    value=st.session_state.analysis_variables['manpower_analysis'].get('receiving_simplified', {}).get('break_time_minutes', 30),
                    step=5,
                    key="receiving_break_time"
                )

        else:
            # Detailed mode — requires time-study inputs
            st.markdown("### 🔄 Detailed Picking Parameters")
            st.caption("Advanced — requires time-study inputs from your warehouse operations.")
            col1, col2 = st.columns(2)
            with col1:
                picking_walk_distance = st.number_input(
                    "Avg Per Pallet Walk Distance (m)",
                    min_value=0.0, max_value=200.0,
                    value=st.session_state.analysis_variables['manpower_analysis']['picking']['avg_walk_distance_per_pallet'],
                    step=1.0,
                    key="picking_walk_distance"
                )
                picking_scan_time = st.number_input(
                    "Scan Time (s)",
                    min_value=0.0, max_value=30.0,
                    value=st.session_state.analysis_variables['manpower_analysis']['picking']['scan_time'],
                    step=0.1,
                    key="picking_scan_time"
                )
            with col2:
                picking_qty_time = st.number_input(
                    "Qty Pick Time (s)",
                    min_value=0.0, max_value=30.0,
                    value=st.session_state.analysis_variables['manpower_analysis']['picking']['qty_pick_time'],
                    step=0.1,
                    key="picking_qty_time"
                )
                picking_misc_time = st.number_input(
                    "Per Pallet Miscellaneous Time (s)",
                    min_value=0.0, max_value=300.0,
                    value=st.session_state.analysis_variables['manpower_analysis']['picking']['misc_time_per_pallet'],
                    step=1.0,
                    key="picking_misc_time"
                )

            st.markdown("### 📥 Receiving and Putaway Parameters")
            col1, col2 = st.columns(2)
            with col1:
                receiving_unloading_time = st.number_input(
                    "Unloading time Per Case (s)",
                    min_value=0.0, max_value=60.0,
                    value=st.session_state.analysis_variables['manpower_analysis']['receiving_putaway']['unloading_time_per_case'],
                    step=0.1,
                    key="receiving_unloading_time"
                )
                receiving_walk_distance = st.number_input(
                    "Avg Per Pallet Walk Distance (m)",
                    min_value=0.0, max_value=200.0,
                    value=st.session_state.analysis_variables['manpower_analysis']['receiving_putaway']['avg_walk_distance_per_pallet'],
                    step=1.0,
                    key="receiving_walk_distance"
                )
            with col2:
                receiving_scan_time = st.number_input(
                    "Scan Time (s)",
                    min_value=0.0, max_value=30.0,
                    value=st.session_state.analysis_variables['manpower_analysis']['receiving_putaway']['scan_time'],
                    step=0.1,
                    key="receiving_scan_time"
                )
                receiving_misc_time = st.number_input(
                    "Miscellaneous Time (s)",
                    min_value=0.0, max_value=300.0,
                    value=st.session_state.analysis_variables['manpower_analysis']['receiving_putaway']['misc_time'],
                    step=1.0,
                    key="receiving_misc_time"
                )

            st.markdown("### 🚛 Loading Parameters")
            col1, col2 = st.columns(2)
            with col1:
                loading_time_per_case = st.number_input(
                    "Loading time per case (s)",
                    min_value=0.0, max_value=60.0,
                    value=st.session_state.analysis_variables['manpower_analysis']['loading']['loading_time_per_case'],
                    step=0.1,
                    key="loading_time_per_case"
                )
            with col2:
                st.write("")

    # ── 3. Percentile Levels ─────────────────────────────────────────────────
    with st.expander("📉 Percentile Levels", expanded=False):
        col1, col2 = st.columns(2)
        with col1:
            st.markdown("**Order Analysis**")
            order_percentile_levels = st.multiselect(
                "Percentiles for Order Analysis",
                options=[99, 95, 90, 85, 80, 75, 70, 65, 60],
                default=st.session_state.analysis_variables['order_analysis']['percentile_levels'],
                key="order_percentile_levels"
            )
        with col2:
            st.markdown("**Receipt Analysis**")
            receipt_percentile_levels = st.multiselect(
                "Percentiles for Receipt Analysis",
                options=[99, 95, 90, 85, 80, 75, 70, 65, 60],
                default=st.session_state.analysis_variables['receipt_analysis']['percentile_levels'],
                key="receipt_percentile_levels"
            )

    # ── 4. Advanced Settings ─────────────────────────────────────────────────
    with st.expander("🌐 Advanced Settings", expanded=False):
        st.markdown("**Pallet Fit Fallback**")
        st.markdown(
            "<small style='color:#666;'>Used only when your SKU master is missing Pallet Fit values. "
            "Set this to the typical cases-per-pallet for your warehouse.</small>",
            unsafe_allow_html=True
        )
        default_pallet_fit = st.number_input(
            "Default Pallet Fit (cases per pallet)",
            min_value=1, max_value=500,
            value=st.session_state.analysis_variables['global'].get('default_pallet_fit', config.DEFAULT_PALLET_FIT),
            step=5,
            help="Fallback value when a SKU has no Pallet Fit in the SKU master.",
            key="default_pallet_fit"
        )


def save_variables_configuration():
    """Save the variables configuration to session state"""

    # Validate thresholds before committing
    abc_a = st.session_state.get('abc_a_threshold', 70)
    abc_b = st.session_state.get('abc_b_threshold', 90)
    fms_f = st.session_state.get('fms_fast_threshold', 70)
    fms_m = st.session_state.get('fms_medium_threshold', 90)
    errors = []
    if abc_a >= abc_b:
        errors.append(f"ABC: A threshold ({abc_a}%) must be less than B threshold ({abc_b}%).")
    if fms_f >= fms_m:
        errors.append(f"FMS: Fast threshold ({fms_f}%) must be less than Medium threshold ({fms_m}%).")
    if errors:
        for e in errors:
            st.error(e)
        return   # do NOT set variables_configured = True

    st.session_state.analysis_variables = {
        'global': {
            'abc_a_threshold': abc_a,
            'abc_b_threshold': abc_b,
            'currency_symbol': '$',
            'decimal_places': 2,
            'working_days_per_week': 5,
            'working_hours_per_day': 8,
            'default_pallet_fit': st.session_state.get('default_pallet_fit', config.DEFAULT_PALLET_FIT),
        },
        'order_analysis': {
            'fms_fast_threshold': fms_f,
            'fms_medium_threshold': fms_m,
            'percentile_levels': st.session_state.get('order_percentile_levels', [95, 90, 85, 80, 75]),
            'vip_customer_threshold': 50,
            'seasonal_adjustment': False,
        },
        'receipt_analysis': {
            'percentile_levels': st.session_state.get('receipt_percentile_levels', [95, 90, 85, 80, 75]),
        },
        'inventory_analysis': {
            'safety_stock_days': 7,
            'reorder_point_days': 14,
            'default_lead_time': 7,
            'storage_cost_percent': 15.0,
        },
        'manpower_analysis': {
            'complexity_mode': st.session_state.get('complexity_mode', 'Simple'),
            'target_efficiency': 85,
            'standard_pick_rate': 60,
            'shifts_per_day': 1,
            'break_time_minutes': 30,
            'picking_simplified': {
                'percentile_for_planning': st.session_state.get('percentile_planning', 95),
                'average_cases_per_pallet': st.session_state.get('cases_per_pallet', 100),
                'average_time_per_pallet': st.session_state.get('time_per_pallet', 30.0),
                'work_efficiency': st.session_state.get('work_efficiency', 85.0),
                'shift_hours': st.session_state.get('shift_hours', 8.0),
                'break_time_minutes': st.session_state.get('break_time', 30),
                'shifts_per_day': 1
            },
            'receiving_simplified': {
                'percentile_for_planning': st.session_state.get('receiving_percentile_planning', 95),
                'average_cases_per_pallet': st.session_state.get('receiving_cases_per_pallet', 100),
                'unloading_putaway_time_per_pallet': st.session_state.get('unloading_putaway_time', 45.0),
                'work_efficiency': st.session_state.get('receiving_efficiency', 85.0),
                'shift_hours': st.session_state.get('receiving_shift_hours', 8.0),
                'break_time_minutes': st.session_state.get('receiving_break_time', 30),
                'shifts_per_day': 1
            },
            'picking': {
                'avg_walk_distance_per_pallet': st.session_state.get('picking_walk_distance', 50.0),
                'scan_time': st.session_state.get('picking_scan_time', 3.0),
                'qty_pick_time': st.session_state.get('picking_qty_time', 2.0),
                'misc_time_per_pallet': st.session_state.get('picking_misc_time', 30.0)
            },
            'receiving_putaway': {
                'unloading_time_per_case': st.session_state.get('receiving_unloading_time', 5.0),
                'avg_walk_distance_per_pallet': st.session_state.get('receiving_walk_distance', 40.0),
                'scan_time': st.session_state.get('receiving_scan_time', 3.0),
                'misc_time': st.session_state.get('receiving_misc_time', 20.0)
            },
            'loading': {
                'loading_time_per_case': st.session_state.get('loading_time_per_case', 4.0)
            }
        }
    }

    st.session_state.variables_configured = True
    st.success("Configuration saved.")


# ──────────────────────────────────────────────────────────────────────────────
# SECTION 3 — Run Analysis
# ──────────────────────────────────────────────────────────────────────────────

def show_analysis_section():
    """Display the analysis execution section"""
    st.markdown('<h2 class="section-header">🚀 Run Analysis</h2>', unsafe_allow_html=True)

    if not st.session_state.file_validated:
        st.warning("Please upload and validate an Excel file first.")
        show_step_navigation_footer(2, "Download Results", next_disabled=True)
        return

    if not st.session_state.variables_configured:
        st.warning("Please configure analysis variables first.")
        show_step_navigation_footer(2, "Download Results", next_disabled=True)
        return

    if st.session_state.analysis_complete:
        # Show results summary and post-run actions
        show_analysis_result_summary()
        st.markdown("---")
        col1, col2 = st.columns(2)
        with col1:
            if st.button("Re-run Analysis (same settings)", type="secondary", use_container_width=True):
                reset_analysis(full_reset=False)
                execute_warehouse_analysis()
        with col2:
            if st.button("Change Variables and Re-run", type="secondary", use_container_width=True):
                reset_analysis(full_reset=False)
                st.session_state.requested_step = 1
                st.rerun()
    else:
        # First run — show readiness summary and Run button
        col1, col2 = st.columns(2)
        with col1:
            st.info(f"📁 **File**: {st.session_state.uploaded_file.name}")
            st.info(f"📊 **Sheets**: {len(st.session_state.file_sheets)} detected")
        with col2:
            available_analyses = config.validate_sheet_availability(st.session_state.file_sheets)
            available_count = sum(available_analyses.values())
            st.info("⚙️ **Variables**: Configured")
            st.info(f"📈 **Available Analyses**: {available_count}")

        st.markdown("### 🎯 Execute Analysis")
        if st.button("🚀 Run Complete Analysis", type="primary", use_container_width=True):
            execute_warehouse_analysis()

    show_step_navigation_footer(
        2, "Download Results",
        next_disabled=not st.session_state.analysis_complete
    )


def _run_analyzer_safe(name: str, analyzer_fn, results_dict: dict) -> bool:
    """
    Run analyzer_fn(), store result in results_dict[name].
    Updates st.session_state.analysis_status and analysis_errors.
    Returns True on success.
    """
    try:
        results_dict[name] = analyzer_fn()
        st.session_state.analysis_status[name] = True
        return True
    except Exception as e:
        results_dict[name] = {'error': str(e), 'success': False}
        st.session_state.analysis_status[name] = False
        st.session_state.analysis_errors[name]  = str(e)
        return False


def execute_warehouse_analysis():
    """Execute the complete warehouse analysis using the analysis modules"""

    progress_bar = st.progress(0)
    status_text  = st.empty()

    try:
        # Step 1: Get data (use cached copy if available — avoids double-load)
        status_text.text("Preparing data...")
        if st.session_state.loaded_data is not None:
            available_data = st.session_state.loaded_data
            progress_bar.progress(20)
        else:
            status_text.text("Loading data from file...")
            progress_bar.progress(10)
            data_loader  = DataLoader(st.session_state.uploaded_file, verbose=False)
            load_results = data_loader.load_all_data()
            if not load_results['success']:
                st.error("Data loading failed:")
                for error in load_results['errors']:
                    st.error(f"• {error}")
                return
            available_data = load_results['data']
            st.session_state.loaded_data = available_data
            progress_bar.progress(20)

        # Step 2: Create analysis configuration
        status_text.text("Preparing analysis configuration...")
        analysis_config = config.create_analysis_config(st.session_state.analysis_variables)

        # Step 3: Run analyses
        analysis_results  = {'data_loader': {'success': True}}
        current_progress  = 20
        sku_master        = available_data.get('sku_master')

        if 'order_data' in available_data:
            status_text.text("Running order pattern analysis...")
            progress_bar.progress(30)
            _run_analyzer_safe(
                'order_analysis',
                lambda: OrderAnalyzer(available_data['order_data'], sku_master, analysis_config).run_complete_analysis(),
                analysis_results
            )
            current_progress = 40

        if 'order_data' in available_data:
            status_text.text("Performing SKU analysis...")
            progress_bar.progress(current_progress)
            _run_analyzer_safe(
                'sku_analysis',
                lambda: SKUAnalyzer(available_data['order_data'], sku_master, analysis_config).run_complete_analysis(),
                analysis_results
            )
            current_progress += 15

        if 'order_data' in available_data:
            status_text.text("Executing ABC-FMS classification...")
            progress_bar.progress(current_progress)
            _run_analyzer_safe(
                'abc_fms_analysis',
                lambda: ABCFMSAnalyzer(available_data['order_data'], sku_master, analysis_config).run_complete_analysis(),
                analysis_results
            )
            current_progress += 15

        if 'receipt_data' in available_data:
            status_text.text("Analyzing receipt patterns...")
            progress_bar.progress(current_progress)
            _run_analyzer_safe(
                'receipt_analysis',
                lambda: ReceiptAnalyzer(
                    available_data['receipt_data'],
                    available_data.get('order_data'),
                    sku_master, analysis_config
                ).run_complete_analysis(),
                analysis_results
            )
            current_progress += 10

        if 'inventory_data' in available_data:
            status_text.text("Analyzing inventory levels...")
            progress_bar.progress(current_progress)
            _run_analyzer_safe(
                'inventory_analysis',
                lambda: InventoryAnalyzer(
                    available_data['inventory_data'],
                    sku_master,
                    available_data.get('order_data'),
                    analysis_config
                ).run_complete_analysis(),
                analysis_results
            )
            current_progress += 10

        if 'order_data' in available_data or 'receipt_data' in available_data:
            status_text.text("Analyzing manpower requirements...")
            progress_bar.progress(current_progress)
            _run_analyzer_safe(
                'manpower_analysis',
                lambda: ManpowerAnalyzer(
                    order_data=available_data.get('order_data'),
                    receipt_data=available_data.get('receipt_data'),
                    sku_master=sku_master,
                    analysis_config=analysis_config
                ).run_complete_analysis(),
                analysis_results
            )
            current_progress += 10

        # Step 4: Generate Excel report
        status_text.text("Generating comprehensive Excel report...")
        progress_bar.progress(85)

        successful_analyses = [
            k for k, v in analysis_results.items()
            if k != 'data_loader' and (not isinstance(v, dict) or v.get('success', True) is not False)
        ]
        if not successful_analyses:
            st.error("No analyses completed successfully. Cannot generate Excel report.")
            return

        try:
            excel_generator = ExcelGenerator(analysis_results, st.session_state.analysis_variables, analysis_config)
            excel_buffer    = excel_generator.generate_comprehensive_report()
        except Exception as e:
            st.error(f"Excel generation failed: {str(e)}")
            if st.checkbox("Show detailed error information"):
                st.code(traceback.format_exc())
            return

        # Step 5: Save to session state
        status_text.text("Analysis complete.")
        progress_bar.progress(100)

        st.session_state.analysis_complete = True
        st.session_state.excel_report_ready = True
        st.session_state.analysis_results = {
            'timestamp':   datetime.now(),
            'file_name':   st.session_state.uploaded_file.name,
            'excel_report': excel_buffer,
            'summary': {
                'analyses_completed': len([k for k in analysis_results if k != 'data_loader']),
                'total_orders':  available_data.get('order_data', pd.DataFrame()).shape[0] if 'order_data' in available_data else 0,
                'unique_skus':   available_data.get('order_data', pd.DataFrame())['Sku Code'].nunique() if 'order_data' in available_data else 0,
                'total_volume':  available_data.get('order_data', pd.DataFrame())['Qty in Cases'].sum() if 'order_data' in available_data else 0
            },
            **analysis_results
        }

        # Show result summary inline (with Go to Download CTA)
        show_analysis_result_summary()

    except Exception as e:
        st.error(f"Analysis failed: {str(e)}")
        if st.checkbox("Show detailed error information"):
            st.code(traceback.format_exc())


def show_analysis_result_summary():
    """Show a structured pass/fail summary of the last analysis run."""
    status = st.session_state.get('analysis_status', {})
    errors = st.session_state.get('analysis_errors', {})

    passed = [k for k, v in status.items() if v]
    failed = [k for k, v in status.items() if not v]

    label_map = {
        'order_analysis':     'Order Analysis',
        'sku_analysis':       'SKU Analysis',
        'abc_fms_analysis':   'ABC-FMS Classification',
        'receipt_analysis':   'Receipt Analysis',
        'inventory_analysis': 'Inventory Analysis',
        'manpower_analysis':  'Manpower Analysis',
    }

    if not failed:
        st.success(f"All {len(passed)} analyses completed successfully.")
    else:
        st.warning(f"{len(passed)} of {len(passed) + len(failed)} analyses completed. {len(failed)} had errors.")

    col1, col2 = st.columns(2)
    with col1:
        if passed:
            st.markdown("**Completed:**")
            for k in passed:
                st.markdown(f"- {label_map.get(k, k)}")
    with col2:
        if failed:
            st.markdown("**Failed (check data):**")
            for k in failed:
                st.markdown(f"- {label_map.get(k, k)}")
            with st.expander("Show error details"):
                for k in failed:
                    st.error(f"**{label_map.get(k, k)}**: {errors.get(k, 'Unknown error')}")

    if st.session_state.excel_report_ready:
        st.markdown("---")
        if st.button("→ Go to Download Results", type="primary", use_container_width=True):
            st.session_state.requested_step = 3
            st.rerun()


# ──────────────────────────────────────────────────────────────────────────────
# SECTION 4 — Download
# ──────────────────────────────────────────────────────────────────────────────

def show_download_section():
    """Display the download results section"""
    st.markdown('<h2 class="section-header">📥 Download Results</h2>', unsafe_allow_html=True)

    if not st.session_state.analysis_complete:
        st.info("No analysis results available. Please run the analysis first.")
        show_step_navigation_footer(3)
        return

    results = st.session_state.analysis_results

    # Summary metrics
    st.markdown("### 📊 Analysis Summary")
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        st.markdown('<div class="metric-container">', unsafe_allow_html=True)
        st.metric("Analysis Date", results['timestamp'].strftime("%Y-%m-%d %H:%M"))
        st.markdown('</div>', unsafe_allow_html=True)
    with col2:
        st.markdown('<div class="metric-container">', unsafe_allow_html=True)
        st.metric("Analyses Completed", results['summary']['analyses_completed'])
        st.markdown('</div>', unsafe_allow_html=True)
    with col3:
        st.markdown('<div class="metric-container">', unsafe_allow_html=True)
        st.metric("Total Orders", f"{results['summary']['total_orders']:,}")
        st.markdown('</div>', unsafe_allow_html=True)
    with col4:
        st.markdown('<div class="metric-container">', unsafe_allow_html=True)
        st.metric("Unique SKUs", results['summary']['unique_skus'])
        st.markdown('</div>', unsafe_allow_html=True)

    # Excel download
    st.markdown("### 📥 Download Excel Report")
    col1, col2 = st.columns([2, 1])
    with col1:
        st.markdown("""
        **Comprehensive Excel Report includes:**
        - Executive Summary with key metrics
        - Order Analysis (patterns, trends, forecasts)
        - SKU Analysis (performance, ABC classification)
        - ABC-FMS Analysis (strategic segmentation)
        - Inventory Analysis (stock levels, recommendations)
        - Receipt Analysis (inbound patterns, efficiency)
        - Consolidated Recommendations
        - Configuration Documentation
        - Raw Data Summary
        """)
    with col2:
        excel_size_kb = len(results['excel_report']) / 1024
        filename = f"Warehouse_Analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
        st.markdown(f"**{excel_size_kb:.0f} KB — 9 sheets**")
        st.download_button(
            label="📥 Download Excel Report",
            data=results['excel_report'],
            file_name=filename,
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            use_container_width=True,
            type="primary"
        )
        st.caption(f"Generated: {results['timestamp'].strftime('%H:%M:%S')}")

    # Word report
    if st.session_state.excel_report_ready:
        st.markdown("---")
        st.markdown("### 📄 Generate MS Word Report")
        col1, col2 = st.columns([2, 1])
        with col1:
            st.markdown("""
            **MS Word Report includes:**
            - Executive Summary with AI-powered insights
            - Receipt Analysis with intelligent commentary
            - Data tables and charts from analysis
            - AI-generated operational recommendations
            - Professional document formatting
            """)
            if not os.getenv('GEMINI_API_KEY'):
                st.warning(
                    "Gemini API key not set — report will use fallback text. "
                    "Set `GEMINI_API_KEY` environment variable for AI insights."
                )
        with col2:
            if st.session_state.word_report_buffer is not None:
                # Already generated — show cached version immediately
                word_size_kb = len(st.session_state.word_report_buffer) / 1024
                st.markdown(f"**{word_size_kb:.0f} KB — ready to download**")
                st.download_button(
                    label="📥 Download Word Report",
                    data=st.session_state.word_report_buffer,
                    file_name=st.session_state.word_report_filename,
                    mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                    use_container_width=True,
                    type="primary"
                )
                if st.button("Regenerate Word Report", type="secondary", use_container_width=True):
                    st.session_state.word_report_buffer   = None
                    st.session_state.word_report_filename = None
                    st.rerun()
            else:
                if st.button("📝 Generate Word Report", type="secondary", use_container_width=True):
                    with st.spinner("Generating Word report with insights..."):
                        try:
                            from scripts.word_report import WordReportGenerator
                            word_gen    = WordReportGenerator(
                                st.session_state.analysis_results,
                                st.session_state.analysis_variables
                            )
                            word_buffer   = word_gen.generate_report()
                            word_filename = f"Warehouse_Report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
                            # Cache so download is instant on re-render
                            st.session_state.word_report_buffer   = word_buffer
                            st.session_state.word_report_filename = word_filename
                            st.rerun()
                        except ImportError:
                            st.error(
                                "Missing dependencies. Run:\n"
                                "```\npip install python-docx google-generativeai python-dotenv matplotlib\n```"
                            )
                        except Exception as e:
                            st.error(f"Error generating Word report: {str(e)}")
                            if "GEMINI_API_KEY" in str(e):
                                st.info("Set GEMINI_API_KEY environment variable for AI insights.")

    show_step_navigation_footer(3)


# ──────────────────────────────────────────────────────────────────────────────
# Template generator (unchanged)
# ──────────────────────────────────────────────────────────────────────────────

def generate_template_excel():
    """Provide the actual test data file as template"""

    test_file_path = "Test Data/ITC_WMS_RAW_DATA.xlsx"

    if os.path.exists(test_file_path):
        with open(test_file_path, 'rb') as f:
            return io.BytesIO(f.read())

    # Fallback to generating sample data if test file not found
    import random
    from datetime import datetime, timedelta

    output = io.BytesIO()

    with pd.ExcelWriter(output, engine='openpyxl') as writer:

        # OrderData Sheet
        start_date = datetime(2025, 1, 1)
        order_data = []

        for i in range(100):
            date = start_date + timedelta(days=random.randint(0, 60))
            order_no    = 144070000 + random.randint(1000, 9999)
            shipment_no = 1008160000 + random.randint(1000, 9999)
            sku_codes   = ['FXT50005W', 'FXT11005I', 'FXT20010B', 'FXT30015G', 'FXT40020R']
            sku_code    = random.choice(sku_codes)
            qty_cases   = random.randint(1, 50)
            qty_eaches  = random.randint(0, 20)
            order_data.append({
                'Date': date,
                'Order No.': order_no,
                'Shipment No.': shipment_no,
                'Sku Code': sku_code,
                'Qty in Cases': qty_cases,
                'Qty in Eaches': qty_eaches
            })

        order_df = pd.DataFrame(order_data)
        order_df.to_excel(writer, sheet_name='OrderData', index=False)

        # SkuMaster Sheet
        sku_master_data = []
        categories = ['CG', 'Food', 'Beverages', 'Personal Care', 'Household']
        for i in range(50):
            sku_master_data.append({
                'Category': random.choice(categories),
                'Sku Code': 12 + i,
                'Case Config': random.choice([250, 500, 1000, 1500]),
                'Pallet Fit': random.choice([8, 10, 12, 15, 20])
            })
        sku_master_df = pd.DataFrame(sku_master_data)
        sku_master_df.to_excel(writer, sheet_name='SkuMaster', index=False)

        # ReceiptData Sheet
        receipt_data = []
        for i in range(75):
            receipt_date = (start_date + timedelta(days=random.randint(0, 60))).strftime('%Y-%m-%d')
            sku_id       = 12 + random.randint(0, 49)
            shipment_no  = f'SH{1000 + random.randint(0, 999)}'
            truck_no     = f'T{random.randint(100, 999):03d}'
            batch        = f'B{2025000 + random.randint(1, 999)}' if random.random() > 0.3 else None
            qty_cases    = random.randint(1, 10)
            qty_eaches   = random.randint(0, 50)
            receipt_data.append({
                'Receipt Date': receipt_date,
                'SKU ID': sku_id,
                'Shipment No': shipment_no,
                'Truck No': truck_no,
                'Batch': batch,
                'Quantity in Cases': qty_cases,
                'Quantity in Eaches': qty_eaches
            })
        receipt_df = pd.DataFrame(receipt_data)
        receipt_df.to_excel(writer, sheet_name='ReceiptData', index=False)

        # InventoryData Sheet
        inventory_data = []
        sites     = ['TWBH', 'MUMBAI', 'DELHI', 'BANGALORE', 'CHENNAI']
        sku_names = [
            'AIM YELLOW 37s', 'Arrow 24s', 'AIM MAXI 60s (WM)', 'Classmate 12s',
            'Sunfeast 24s', 'Bingo 36s', 'Aashirvaad 48s', 'Fiama 60s',
            'Vivel 24s', 'Savlon 12s', 'Boost 18s', 'Horlicks 24s'
        ]
        for i in range(80):
            calendar_day = (start_date + timedelta(days=random.randint(120, 150))).strftime('%d.%m.%Y')
            site         = random.choice(sites)
            sku_id       = 9000 + random.randint(100, 999)
            sku_name     = random.choice(sku_names)
            stock_cases  = random.randint(0, 200)
            stock_pieces = round(random.uniform(0, 100), 1)
            inventory_data.append({
                'Calendar Day': calendar_day,
                'Site': site,
                'SKU ID': sku_id,
                'SKU Name': sku_name,
                'Total Stock in Cases (In Base Unit of Measure)': stock_cases,
                'Total Stock in Pieces (In Base Unit of Measue)': stock_pieces
            })
        inventory_df = pd.DataFrame(inventory_data)
        inventory_df.to_excel(writer, sheet_name='InventoryData', index=False)

    output.seek(0)
    return output.getvalue()


if __name__ == "__main__":
    main()
